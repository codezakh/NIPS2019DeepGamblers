...
# Need to do this to fix a bug in NumPy.
...
class ImgAugWrapper:
    ...
    def __call__(self, img):
        ...
        return Image.fromarray(self.augmentor.augment_image(img_arr))

model_names = ("vgg16", "vgg16_bn")

parser = argparse.ArgumentParser(
    description="PyTorch cifar10 / svhn / catsdogs Gambler training"
)
parser.add_argument(
    "-d",
    "--dataset",
...
parser.add_argument(
    "--test-batch", default=200, type=int, metavar="N", help="test batchsize"
)
parser.add_argument(
    "--lr",
    "--learning-rate",
...
parser.add_argument(
    "--fastdebug",
    action="store_true",
    help="A fast debug option. If this is set, we break after one batch.",
)
...
def main():
    ...
    if args.dataset == "cifar10":
        ...
        transform_test = transforms.Compose(
            [
                transforms.RandomApply(
                    [
                        ImgAugWrapper(iaa.SaltAndPepper(0.99))
                    ],
                    p=0.15
                ),
                transforms.ToTensor(),
        ...
        assert os.path.exists("./data/cats_dogs/train") and os.path.exists(
            "./data/cats_dogs/test"
        ), "Please download and put the 'cats vs dogs' dataset to paths 'data/cats_dogs/train' and 'cats_dogs/test'"
    ...
    model = models.__dict__[args.arch](
        num_classes=num_classes + 1, input_size=input_size
    )
    ...
    logger.set_names(
        ["Epoch", "Learning Rate", "Train Loss", "Test Loss", "Train Err.", "Test Err."]
    )
...
def test(testloader, model, criterion, epoch, use_cuda, evaluation=False):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            if epoch >= args.pretrain:
                ...
                # calculate loss
                gain = torch.gather(
                    outputs, dim=1, index=targets.unsqueeze(1)
                ).squeeze()
                doubling_rate = (gain.add(reservation.div(reward))).log()
                loss = -doubling_rate.mean()
            else:
    ...
    if epoch >= args.pretrain:
        ...
        for coverage in expected_coverage:
            print(
                "{:.0f}: {:.3f}, ".format(
                    coverage, sum(sorted_correct[int(size / 100 * coverage) :])
                ),
                end="",
            )
        print("")
...
def validation(testloader, model, use_cuda):
    ...
    for idx, result in enumerate(results):
        f.write(
            "valid{:.0f},{:.2f},{:.3f}\n".format(
                expected_coverage[idx],
                results_valid[idx][0] * 100.0,
...
def bisection_method(abortion, correct, results):
    ...
    while True:
        ...
        if passed_up / len(correct) * 100.0 < expected_coverage[0]:
            upper *= 2.0
        else:
            break
    test_thres = 1.0
    for coverage in expected_coverage:
        ...
        lower = 0.0
        while math.fabs(passed / len(correct) * 100.0 - coverage) > 0.3:
            if passed / len(correct) * 100.0 > coverage:
                upper = min(test_thres, upper)
                test_thres = (test_thres + lower) / 2
            elif passed / len(correct) * 100.0 < coverage:
...
def save_data():
    ...
    for reward in reward_list:
        ...
        if os.path.isfile(os.path.join(save_path, "coverage VS err.csv")):
            ...
            print(
                "no file exists at {}".format(
                    os.path.join(save_path, "coverage VS err.csv")
                )
            )
            continue
        content = f.read()
